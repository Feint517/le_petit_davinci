...
class FrenchSentenceConstruction extends StatefulWidget {
  ...
  @override
  State<FrenchSentenceConstruction> createState() => _FrenchSentenceConstructionState();
}

class _FrenchSentenceConstructionState extends State<FrenchSentenceConstruction> {
  ...
  final List<String> _initialDraggableWords = ['lire',"J'aime", 'livres', 'des'];
        ...
        child: Column(spacing:10,
          children: [
            TopNavigation(text: 'Français', buttonColor: AppColors.bluePrimaryDark), 
             ...
             Text('Phrase à reconstruire', style: TextStyle(
              color: AppColors.textPrimary,
              fontSize: 24,
              fontWeight: FontWeight.bold,),),
              Text("Exercer la syntaxe et l'ordre des mots.", style: TextStyle(
               ...
               child: Stack( 
                ...
                Align(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.center,spacing: 20,mainAxisAlignment: MainAxisAlignment.start,
                      children: [ 
                    const Text(
                      'Glisser ces mots:',
                    ...
                    Wrap(
                      ...
                      children: _draggableWords.map((word) {
                        return Padding(
                                  padding: const EdgeInsets.all(4.0), // Smaller padding for each word
                                  child: Draggable<String>(
                                    ...
                                    childWhenDragging: Container(
                                      ...
                                      padding: const EdgeInsets.all(12),
                                      decoration: BoxDecoration(
                                        color: Colors.grey[700], // Slightly darker to show it's gone
                                        borderRadius: BorderRadius.circular(8),
                                      ),
                                      child: Text(
                                        word,
                    ...
                    Row(
                      ...
                      children: List.generate(_droppedWords.length, (index) {
                        return SizedBox(
                          width: 80, // Fixed width for input fields
                          child: DragTarget<String>(
                            onWillAccept: (data) => true, // Always allow dropping
                            onAccept: (data) {
                              setState(() {
                                        // If the slot is already filled, add the word back to draggable
                                        if (_droppedWords[index].isNotEmpty) {
                                          _draggableWords.add(_droppedWords[index]);
                                        }
                                        _droppedWords[index] = data; // Place the dropped word
                                        _draggableWords.remove(data); // Remove from draggable list
                                        _feedbackMessage = ''; // Clear feedback on new drop
                                        _isCorrectAnswer = null;
                              });
                            },
                            builder: (context, candidateData, rejectedData) {
                              return TextField(
                                        controller: TextEditingController(text: _droppedWords[index]),
                                        textAlign: TextAlign.center,
                                        enabled: false, // Make it non-editable by keyboard
                                        style: const TextStyle(color: AppColors.white, fontSize: 16),
                    ...
                    // Action Buttons
                     CustomButtonNew(buttonColor: AppColors.secondary, shadowColor: AppColors.orangeAccentDark, label: 'Vérifier ma réponse', labelColor: AppColors.background,
                         onPressed: _checkAnswer,icon: Icons.arrow_outward_rounded,iconColor: AppColors.backgroundLight,width: context.width/1.5,), 
                         CustomButtonNew(buttonColor: AppColors.orange, shadowColor: AppColors.pinkMedium, label: 'Reset', labelColor: AppColors.background,
                         onPressed: _initializeGame,icon: Icons.refresh_rounded,iconColor: AppColors.backgroundLight,width: context.width/1.5,), 
                     
                    const SizedBox(height: 20), // Space before feedback message
                    // Feedback Message
                    if (_feedbackMessage.isNotEmpty)
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12),
                        decoration: BoxDecoration(
                          color: _isCorrectAnswer == true ? Colors.green.shade700 : Colors.red.shade700,
